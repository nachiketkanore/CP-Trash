...
vector<int> power(vector<int>& v, int exponent) {
	assert(exponent >= 0);
	vector<int> result = { 1 };
	...
	while (exponent > 0) {
		...
		trim(v);
		trim(result);
		// what happened was after some K, all numbers may get > MX
		// so result or v after trimming could become empty
		// we need to stop exponentiation there
		if (result.empty() || v.empty())
			return result;
		// cerr << sz(v) << ' ' << sz(result) << " <-- checking\n";
	}
	return result;
}
}; // namespace NTT
...
void assert_range(int a, int L, int R) {
	assert(L <= a && a <= R);
}
...
int32_t main() {
	...
	int N, K;
	cin >> N >> K;
	assert_range(N, 1, 100000);
	assert_range(K, 1, 100000);
	vector<int> A(MX);
	F0R(i, N) {
		int u;
		cin >> u;
		assert_range(u, 1, 100000);
		A[u]++;
	}
	vector<int> P = NTT::power(A, K);
	// print_polynomial(P);
	// NTT::trim(P);
	while (sz(P) < MX)
		P.push_back(0);
	...
	int Q;
	cin >> Q;
	assert_range(Q, 1, 100000);
	while (Q--) {
		int L, R;
		cin >> L >> R;
		assert(L <= R);
		assert_range(L, 1, 100000);
		assert_range(R, L, 100000);
		mi nume = mi(P[R]) - mi(P[L - 1]);
		// see(nume, deno);
		mi ans = nume / deno;
		// cout << nume << ' ' << deno << '\n';
		cout << ans << '\n';
