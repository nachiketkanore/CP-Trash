...
int32_t main() {
	...
	for (int i = 0; i < sz(primes); i++) {
		auto one = power(primes[i],2);	if (!one.first) break;
		for (int j = 0; j < sz(primes); j++) {
			auto two = power(primes[j],3);	if (!two.first) break;
			for (int k = 0; k < sz(primes); k++) {
				auto three = power(primes[k],4);	if (!three.first) break;
