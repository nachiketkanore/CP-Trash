...
Created: Thursday 17 December 2020 03:00:05 PM IST
...
int32_t main() {
	...
	auto f = [&](vector<int> a) {
		...
		while (sz(a) > 1) {
			assert(__builtin_popcountll(sz(a)) == 1);
			// is power of 2
			int add = 0;
			vector<int> next_a;
			for (int i = 0; i < sz(a); i += 2) {
				add += abs(a[i] - a[i+1]);
				next_a.push_back(max(a[i], a[i+1]));
			}

			// see(a, add);

			ans += add;
			assert(sz(next_a) == sz(a) / 2);
			a = next_a;
		}
