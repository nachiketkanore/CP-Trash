...
Created: Monday 21 December 2020 12:07:58 PM IST
(ツ) If you want a thing done well, do it yourself.
*/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
int go(int id, int prev) {
	if (id == n+1) return 0;
	int &ans = dp[prev][id];	if (~ans) return ans;
	ans = inf;
	if (1 ^ prev)
		cmin(ans, cost[a[id]][1] + go(id+1, 1));
	if (0 ^ prev)
		cmin(ans, cost[a[id]][0] + go(id+1, 0));
	return ans;
}

void solve() {
	cin >> n;
	cin >> cost[0][1] >> cost[1][0];
	FOR (i,1,n) cin >> a[i], dp[0][i] = dp[1][i] = -1;
	int ans = inf;
	cmin(ans, cost[a[1]][0] + go(2, 0));
	cmin(ans, cost[a[1]][1] + go(2, 1));
	assert(ans ^ inf);
	cout << ans << "\n";
}
