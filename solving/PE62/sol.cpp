...
Created: Wednesday 16 December 2020 07:36:25 PM IST
...
map<array<int,10>, vector<int>> cubes;
...
int32_t main() {
	...
	for (int i = 1; i <= 1e6; i++) {
		int cube = 1LL * i * i * i;
		int val = cube;
		array<int,10> f;	for (int j = 0; j < 10; j++) f[j] = 0;
		while (cube > 0) {
			f[cube % 10]++;
			cube /= 10;
		}
		cubes[f].push_back(val);
	}
	...
	for (auto &it : cubes) {
		if (sz(it.second) == 5) {
			...
			valid_answers.push_back(small);
		}
	}

	sort(ALL(valid_answers));
	see(valid_answers.front());
}
