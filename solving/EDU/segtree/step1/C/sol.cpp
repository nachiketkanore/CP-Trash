...
Created: Wednesday 02 December 2020 08:26:10 PM IST
(ツ) Wisdom is knowing what to do next; Skill is knowing how ot do it, and Virtue is doing it.
*/
#include <iostream>
#include <algorithm>
#include <vector>
...
struct segtree {
	...
	void merge(node &ret, node &left, node &right) {
		...
		if (left.mn < right.mn) 
			ret.mn_cnt = left.mn_cnt;
		else if (right.mn < left.mn)
			ret.mn_cnt = right.mn_cnt;
		else
			ret.mn_cnt = left.mn_cnt + right.mn_cnt;
	}
...
int32_t main() {
	...
	while (Q--) {
		...
		if (tc == 1) {
			...
			L++, R++;
			node answer = T.query(L, R-1);
			cout << answer.mn << " " << answer.mn_cnt << "\n";
		}
	}
}
