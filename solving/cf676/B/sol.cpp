...
int32_t main() {
	...
	while (T--) {
		...
		if (one == two) {
			if (one == 0) {
				if (three == 0) ans.push_back({n, n - 1});
				if (four == 0) ans.push_back({n - 1, n});
			} else if (one == 1) {
				if (three == 1) ans.push_back({n, n - 1});
				if (four == 1) ans.push_back({n - 1, n});
			} else assert(false);
		} else if (three == four) {
			if (three == 0) {
				if (one == 0) ans.push_back({2, 1});
				if (two == 0) ans.push_back({1, 2});
			} else if (three == 1) {
				if (one == 1) ans.push_back({2, 1});
				if (two == 1) ans.push_back({1, 2});
			} else assert(false);
		} else {
