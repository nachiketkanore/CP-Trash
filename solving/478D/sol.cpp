...
int main() {
	...
	for (int i = 900; i >= 0; i--) {
		...
		for (int j = 0; j <= g; j++) {
			...
			if (j > i)
				dp[i % 2][j] = max(dp[i % 2][j], dp[(i + 1) % 2][j - i - 1] + 1);
		}
	}
	int h = dp[0][g];
...
int main() {
	...
	for (int i = 1000; i > 0; i--) {
		for (int R = 0; R <= r; R++) {
			...
			if (!(G < 0 || R < 0) && !(G < i && R < i))
				Dpm[i & 1][R] = max(max(Dpm[(i + 1) & 1][R], R - i >= 0 ? Dpm[(i + 1) & 1][R - i] : 0), i);
		}
	}
	...
	for (int i = 1000; i > 0; i--) {
		for (int R = 0; R <= r; R++) {
			...
			if (!(i > h) && !(G < 0 || R < 0))
				Dpc[i & 1][R] = (Dpc[(i + 1) & 1][R] + (R - i >= 0 ? Dpc[(i + 1) & 1][R - i] : 0)) % Mod;
		}
	}
