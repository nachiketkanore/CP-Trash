...
Created: Tuesday 15 December 2020 05:06:43 PM IST
(ツ) Only those who dare to fail greatly can ever achieve greatly.
*/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
int32_t main() {
	...
	for (int luck : lucky) {
		if (L1 <= luck && luck <= R1) one.pb(luck);
		if (L2 <= luck && luck <= R2) two.pb(luck);
	}
}
