...
Created: Saturday 19 December 2020 03:34:34 PM IST
(ツ) I can't imagine a person becoming a success who doesn't give this game of life everything hes got.
*/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
int go(int id, int rem) {
	if (rem < 0) return 0;
	if (id == 0) return rem == 0;
	int &ans = dp[id][rem];
	if (~ans) return ans;
	ans = 0;
	for (int nid = id; nid >= 1; nid--) {
		ans += go(nid-1, rem-1);
	}
	return ans;
}
