...
Created: Tuesday 15 December 2020 01:59:45 PM IST
(ツ) The price of anything is the amount of life you exchange for it.
*/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
int32_t main() {
	...
	while (Q--) {
		...
		if (type == 1) {
			...
			int mod, under;
			cin >> under >> mod;
			if (under <= K) cout << sum[mod][under] << "\n";
			else {
				int ans = 0;
				for (int i = mod; i <= N; i += under)
					ans += a[i];
				cout << ans << "\n";
			}
		}
