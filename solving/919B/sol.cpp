...
Created: Tuesday 29 December 2020 12:31:49 AM IST
(ツ) To be thoughtful and kind only takes a few seconds compared to the timeless hurt caused by one rude gesture.
*/
#include <iostream>
#include <cstdio>
#include <cstdlib>
...
int32_t main() {
	...
	queue<array<int,2>> Q;
	FOR (i,1,9) Q.push({i,i});

	while (!Q.empty()) {
		auto [num, sod] = Q.front();
		Q.pop();
		if (sod > 10) continue;
		if (sod == 10) {
			valid.push_back(num);
		...
		for (int dig = 0; dig < 10; dig++) {
			int nxt_num = num * 10 + dig;
			int nxt_sod = sod + dig;
			Q.push({nxt_num, nxt_sod});
		}
	}
