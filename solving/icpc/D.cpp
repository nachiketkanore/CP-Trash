...
int32_t main() {
	...
	auto add_graph = [&](Dinic& flower) {
		...
		FOR (i,1,n) FOR (j,1,m) {
			flower.add_edge(in[i][j], out[i][j], 1);
		}
	};
	...
	F0R (rev,2) F0R (mask,1<<4) if (__builtin_popcount(mask) == 2) {
		...
		F0R (j,4) {
			if (rev) {
				if (mask >> j & 1) starts.push_back(border_pts[j]);
				else ends.push_back(border_pts[j]);
			} else {
				if (mask >> j & 1) ends.push_back(border_pts[j]);
				else starts.push_back(border_pts[j]);
			}
		}
