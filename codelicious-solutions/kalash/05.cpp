...
template <typename node> struct segtree {
	...
	template <typename... M> void modify(int ql, int qr, const M&... v) {
		assert(0 <= ql && ql <= qr && qr <= n - 1);
...
void solve() {
	...
	for (int i = 1; i <= n; ++i) {
		b[intime[i]] = a[i - 1];
		// assert (outime[i] - intime[i] == sub[i] - 1);
	}
	// assert (euler.size() == n);
	...
	for (int i = 0; i < q; ++i) {
		...
		cin >> t >> u >> p;
		if (t == 1) {
			dev.modify(intime[u], intime[u], p);
		} else {
			if (p > sub[u]) {
				cout << -1 << endl;
				continue;
			...
			long long ans = 0;
			for (int i = 0; i < p; ++i) {
				auto U = dev.get(intime[u], outime[u]);
				opt.push_back({ U.mx, U.idx });
				ans += U.mx;
				dev.modify(U.idx, U.idx, INT_MIN);
			}
			for (auto [v, idx] : opt) {
				dev.modify(idx, idx, v);
			}
