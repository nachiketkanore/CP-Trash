...
bool inside(int i, int j) { return i >= 0 && i < H && j >= 0 && j < W; }
...
void solve() {
	sort(ALL(B), [&](building one, building two) {
		return make_pair(one.x, one.y) > make_pair(two.x, two.y);
	});
	sort(ALL(A), [&](antenna one, antenna two) {
		if (one.C == two.C) return one.R < two.R;
		return one.C < two.C;
	});

	for (building b : B) {
		if (!sz(A) || sz(answer) == M) break;
		int id = sz(A) - 1;
