...
void solve() {
	sort(ALL(B), [&](building one, building two) { return one.C < two.C; });
	sort(ALL(A), [&](antenna one, antenna two) { return one.C > two.C; });
	for (building b : B) {
		answer.push_back(ans(A.back().id, b.x, b.y));
		A.pop_back();
	}
}

void score() {
	assert(sz(answer) == M && M == N);
	// for (building b : B) {
	// 	REP(x,)
	// }
}
