...
int32_t main() {
	...
	set<int> available;
	for (int i = 1; i <= N; i++) {
		if (!binary_search(blues.begin(), blues.end(), i) &&
			!binary_search(reds.begin(), reds.end(), i)) {
			available.insert(i);
		}
	}
	// for (int x: available) {
	...
	for (int i = (int)reds.size() - 1; i >= 0; ) {
		int j = i - 1, cnt = 0;
		while (j >= 0 && reds[j] == reds[j + 1]) cnt++, j--;
		while (cnt--) {
			if (available.empty()) {
				ok = false;
				break;
	...
	for (int i = 0; i < (int)blues.size(); ) {
		int j = i + 1, cnt = 0;
		while (j < (int)blues.size() && blues[j] == blues[j - 1]) cnt++, j++;
		while (cnt--) {
			if (available.empty()) {
				ok = false;
				break;
