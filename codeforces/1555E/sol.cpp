 ...
 *    Created: Thursday 02 June 2022 12:05:55 PM IST
 **/
#include "bits/stdc++.h"
using namespace std;
...
int32_t main() {
	...
	auto gap_exists = [&]() -> bool {
		int gap = segs.query(1, M - 1).mn;
		return gap == 0;
	};

	auto add_segment = [&](int L, int R) -> void { segs.update(L, R - 1, +1); };

	auto remove_segment = [&](int L, int R) -> void { segs.update(L, R - 1, -1); };

	for (int i = 0, j = 0; i < N; i++) {
		while (j < N && gap_exists()) {
			add_segment(A[j][1], A[j][2]);
			j++;
		}
