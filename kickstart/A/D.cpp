...
int solve(int k)
{
	if (already.count(k)) return already[k];
	int K = k;
	if (k <= 10) return k;
	sz = 0;
...
int32_t main(){
	...
	for (int i = 1; i < 10; i++) {
		int val = i;
		while (val % 2 == 0) cnt[i][2]++, val /= 2;
		while (val % 3 == 0) cnt[i][3]++, val /= 3;
		while (val % 5 == 0) cnt[i][5]++, val /= 5;
		while (val % 7 == 0) cnt[i][7]++, val /= 7;
	}
